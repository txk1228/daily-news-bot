...
默认三个主题在国内回退源上带有同义词过滤（如「大模型」「人形机器人」「A股」等）。
...
```text
优先：主题专业源（如「每日财经热点」→ 虎嗅 / 36氪）
   ↓ 不足时
Google News（使用定制检索词，如虎嗅/华尔街见闻/财联社）
   ↓ 再不足
通用回退：IT之家 / 36氪 / Solidot + 同义词过滤
   ↓
每主题最多 3 条，飞书消息内按 1）2）3）分点列出
```
...
**Q: 提示「未获取到任何新闻，任务终止」？**  
Google News 可能暂时不可达，且国内源未命中主题词。稍后再跑，或换更宽的 `--topics`。
